...
/* 
对比工厂模式：
    1.对比于工厂模式，构造函数不需要new Object(),系统会自动创建 var obj = new Object()。
...
构造函数模式使用规则：
    ...
    2.需要使用new运算符来创建实例对象，而工厂模式是不需要的。
    3.也可以使用对象冒充方法来调用
*/
...
//console.log(box1.run());           //lee100运行中。。     //用实例对象box1调用构造函数Box的run方法
//console.log(box2.run());           //jack200运行中。。    //用实例对象box2调用构造函数Box的run方法
//console.log(box3.build());         //kkk500运行中。。     //用实例对象box3调用构造函数Desk的do方法
//console.log(o_Desk.build());       //sweet20运行中。。    //用冒充对象调用Desk的build方法

/* 可以通过这个方法判断实例对象属于哪一个构造函数从而判断类型 */
/* console.log(box1 instanceof Object);    //true  
console.log(box1 instanceof Box);       //true              
console.log(1);
console.log(box2 instanceof Desk);      //false
console.log(2);
...
console.log(typeof box1.run());           //string        //有括号就代表着返回方法里面的值
console.log(typeof box1.run);             //function      //没有括号就代表着返回方法的位置，更深一层就是堆内存里面的地址

console.log(box1.run() === box2.run());  //值都是lee100运行中。。所以是true   【这里比较值和类型是否都相等】                  